...
{p, list, div} = dc
{getCommonRoot, getComponentsBetweenPair} = require('dc-util/addon')
...
describe "dc-util/addon", ->
  ...
  describe "getComponentsBetweenPair", ->
    it 'should getComponentsBetweenPair of two same component', ->
      comp = p()
      expect(getComponentsBetweenPair(comp, comp)).to.deep.equal([comp])

    it 'should getComponentsBetweenPair of two component', ->
      comp = list(c1=p(), c2=p())
      expect(getComponentsBetweenPair(c1, c2)).to.deep.equal([comp])

    it 'should getComponentsBetweenPair of two component in different level', ->
      comp = div(c1=p(), c4 = list(p(), c2=p()))
      lst = getComponentsBetweenPair(c1, c2)
      expect(lst).to.deep.equal([comp])

    it 'should getComponentsBetweenPair of two component in different level 2', ->
      comp = div(comp1 = div(c1=p(), c4=div(p(), c2=p())), p())
      expect(getComponentsBetweenPair(c1, c2)).to.deep.equal([comp1])

    it 'start is not first leaf', ->
      comp = div(comp1 = div(p(), c1 = p(), c4 = div(p(), c2=p())), p())
      expect(getComponentsBetweenPair(c1, c2)).to.deep.equal([c1, c4])

    it 'start is not first leaf with mid brother', ->
      comp = div(comp1 = div(p(), c1 = p(), c3 = p(), c4 = div(p(), c2=p())), p())
      expect(getComponentsBetweenPair(c1, c2)).to.deep.equal([c1, c3, c4])

    it 'start is first leaf with left top', ->
      comp = div(comp1 = div(c4 = div(c5 = p(), c2=p()), c1 = p(), p()))
      expect(getComponentsBetweenPair(c5, c1)).to.deep.equal([c4, c1])
