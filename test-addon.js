var ddescribe, div, expect, getCommonRoot, getComponentsBetweenPair, idescribe, iit, list, ndescribe, nit, p, _ref, _ref1;
...
_ref1 = require('dc-util/addon'), getCommonRoot = _ref1.getCommonRoot, getComponentsBetweenPair = _ref1.getComponentsBetweenPair;
...
describe("dc-util/addon", function() {
  ...
  return describe("getComponentsBetweenPair", function() {
    it('should getComponentsBetweenPair of two same component', function() {
      var comp;
      comp = p();
      return expect(getComponentsBetweenPair(comp, comp)).to.deep.equal([comp]);
    });
    it('should getComponentsBetweenPair of two component', function() {
      var c1, c2, comp;
      comp = list(c1 = p(), c2 = p());
      return expect(getComponentsBetweenPair(c1, c2)).to.deep.equal([comp]);
    });
    it('should getComponentsBetweenPair of two component in different level', function() {
      var c1, c2, c4, comp, lst;
      comp = div(c1 = p(), c4 = list(p(), c2 = p()));
      lst = getComponentsBetweenPair(c1, c2);
      return expect(lst).to.deep.equal([comp]);
    });
    it('should getComponentsBetweenPair of two component in different level 2', function() {
      var c1, c2, c4, comp, comp1;
      comp = div(comp1 = div(c1 = p(), c4 = div(p(), c2 = p())), p());
      return expect(getComponentsBetweenPair(c1, c2)).to.deep.equal([comp1]);
    });
    it('start is not first leaf', function() {
      var c1, c2, c4, comp, comp1;
      comp = div(comp1 = div(p(), c1 = p(), c4 = div(p(), c2 = p())), p());
      return expect(getComponentsBetweenPair(c1, c2)).to.deep.equal([c1, c4]);
    });
    it('start is not first leaf with mid brother', function() {
      ...
      return expect(getComponentsBetweenPair(c1, c2)).to.deep.equal([c1, c3, c4]);
    });
    it('start is first leaf with left top', function() {
      var c1, c2, c4, c5, comp, comp1;
      comp = div(comp1 = div(c4 = div(c5 = p(), c2 = p()), c1 = p(), p()));
      return expect(getComponentsBetweenPair(c5, c1)).to.deep.equal([c4, c1]);
    });
    ...
    it('more complicated', function() {
      ...
      return expect(getComponentsBetweenPair(c5, c1)).to.deep.equal([c5, c2, c1]);
    });
    it('more complicated 2', function() {
      var c1, c2, c4, c5, c6, comp, comp1;
      comp = div(comp1 = div(p(), c4 = div(c6 = p(), c5 = p(), c2 = p()), div(c6 = p(), c1 = p(), p()), p()));
      return expect(getComponentsBetweenPair(c5, c1)).to.deep.equal([c5, c2, c6, c1]);
    });
    it('more complicated 3', function() {
      ...
      return expect(getComponentsBetweenPair(c5, c1)).to.deep.equal([c5, c2, c7, c6, c1]);
    });
    it('more complicated 4', function() {
      ...
      return expect(getComponentsBetweenPair(c5, c1)).to.deep.equal([c5, c2, c7, c8, c6, c1]);
    });
    it('more complicated 5', function() {
      ...
      return expect(getComponentsBetweenPair(c5, c1)).to.deep.equal([c5, c2, c9, c7, c8, c6, c1]);
    });
    it('more complicated 6', function() {
      ...
      comp.mount();
      expect(getComponentsBetweenPair(c3, c1)).to.deep.equal([c11, c7, c8, c6, c1]);
      return comp.unmount();
    });
    return it('more complicated 7', function() {
      ...
      comp.mount();
      expect(getComponentsBetweenPair(c3, c1)).to.deep.equal([c11, c7, c12]);
      return comp.unmount();
    });
  });
});
